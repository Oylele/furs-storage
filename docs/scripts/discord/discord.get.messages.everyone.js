...
let discordEveryoneMessages = {
	...
	out: async function (server, channel, limit = '', token = '', view = 'feed') {
		...
		let mentions, mention_id, mention_name, mention_everyone;
		...
		for (; i < j; i++) {
			...
			mention_everyone = messages[i].mention_everyone;
			...
			content = discord.parser.url(content);
